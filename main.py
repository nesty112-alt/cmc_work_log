...
def generate_html_report(date_input, target_dir, daily_dict, monthly_dict, memo_content, staff_list, task_categories, is_preview=False, df_monthly_base=None):
    ...
    try:
        ...
        weekdays = ["?îÏöî??, "?îÏöî??, "?òÏöî??, "Î™©Ïöî??, "Í∏àÏöî??, "?†Ïöî??, "?ºÏöî??]
    ...
    task_daily_datasets = [{"label": "?ºÏùº ?©Í≥Ñ", "data": task_daily_totals, "backgroundColor": task_bar_colors}]
    task_monthly_datasets = [{"label": "?ÑÏ†Å ?©Í≥Ñ", "data": task_monthly_totals, "backgroundColor": task_bar_colors}]
    ...
    if df_monthly_base is not None and not df_monthly_base.empty:
        import pandas as pd
        task_cols = [c for c in df_monthly_base.columns if c not in ['?ºÏûê', '?¥Îãπ??, 'other_progress', 'req_in_progress', 'req_out_progress', 'mail_progress']]
        df_trend = df_monthly_base[['?ºÏûê'] + task_cols].copy()
        
        df_trend[task_cols] = df_trend[task_cols].apply(pd.to_numeric, errors='coerce').fillna(0)
        df_trend_daily = df_trend.groupby('?ºÏûê')[task_cols].sum()
            ...
            "options": {
                ...
                "scales": { 
                    ...
                    "y": { 
                        ...
                        "max": y_max, 
                        "title": {"display": True, "text": "?ºÏùº ?ÖÎ¨¥??, "font": { "size": 10 }},
                        "ticks": { "font": { "size": 10 } }
                    },
                    "y1": { 
                        ...
                        "title": {"display": True, "text": "?ÑÏ†Å ?ÖÎ¨¥??, "font": { "size": 10 }},
                        "ticks": { "font": { "size": 10 } }
                    }
                }
            }
    ...
    daily_img_tag = f'<img id="dailyChartFallback" src="{daily_b64}" style="width:100%; max-width:700px; display:block; margin:0 auto;" alt="?ºÏùº ?§Ï†Å Ï∞®Ìä∏">' if daily_b64 else ''
    monthly_img_tag = f'<img id="monthlyChartFallback" src="{monthly_b64}" style="width:100%; max-width:700px; display:block; margin:0 auto;" alt="???ÑÏ†Å ?§Ï†Å Ï∞®Ìä∏">' if monthly_b64 else ''
    task_daily_img_tag = f'<img id="taskDailyChartFallback" src="{task_daily_b64}" style="width:100%; max-width:700px; display:block; margin:0 auto;" alt="?ÖÎ¨¥Î≥??ºÏùº ?§Ï†Å Ï∞®Ìä∏">' if task_daily_b64 else ''
    task_monthly_img_tag = f'<img id="taskMonthlyChartFallback" src="{task_monthly_b64}" style="width:100%; max-width:700px; display:block; margin:0 auto;" alt="?ÖÎ¨¥Î≥??ÑÏ†Å ?§Ï†Å Ï∞®Ìä∏">' if task_monthly_b64 else ''
    trend_img_tag = f'<img id="trendChartFallback" src="{trend_b64}" style="width:100%; display:block; margin:0 auto;" alt="?ÑÏ†Å Ï∂îÏù¥ Í∑∏Îûò??>' if trend_b64 else ''
    ...
    <table class="report-table">
        <thead>
            <tr>
                <th rowspan="2">?ÖÎ¨¥Î∂ÑÎ•ò</th>
                <th rowspan="2">?ÖÎ¨¥Î™?/th>
    """
    for staff in staff_list:
        table_html += f'<th colspan="2">{staff}</th>'
    table_html += "</tr><tr>"
    for _ in staff_list:
        table_html += "<th>?ºÍ≥Ñ</th><th>?ÑÍ≥Ñ</th>"
    ...
    try:
        ...
        html_content = html_content.replace("{{PAGE_TITLE}}", f"?¨Ïõê?êÍ? ?¥ÏõêÎ∂ÑÏÑù ?ÖÎ¨¥ ?Ä?úÎ≥¥??{file_date}")
        try:
        ...
        html_dir = os.path.join(target_dir, "html ?Ä?úÎ≥¥??)
        os.makedirs(html_dir, exist_ok=True)
        final_path = os.path.join(html_dir, f"?¨Ïõê?êÍ? ?¥ÏõêÎ∂ÑÏÑù ?ÖÎ¨¥ ?Ä?úÎ≥¥??{file_date}.html")
...
class FileLock:
    ...
    def __enter__(self):
        start = time.time()
        # ?¥ÎçîÍ∞Ä ?ÜÏúºÎ©??ùÏÑ± (Errno 2 Î∞©Ï?)
        os.makedirs(os.path.dirname(self.lockfile), exist_ok=True)
        while True:
            try:
                # ?êÏûê???ùÏÑ±?ºÎ°ú ?ôÏãú ?ëÍ∑º Ï∞®Îã®
                fd = os.open(self.lockfile, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
                os.close(fd)
                break
            except FileExistsError:
                if time.time() - start > self.timeout:
                    raise TimeoutError(f"?åÏùº ?†Í∏à ?ÄÍ∏??úÍ∞Ñ Ï¥àÍ≥º: ?§Î•∏ ?¨Ïö©?êÍ? ?¨Ïö© Ï§ëÏùº ???àÏäµ?àÎã§.\n({self.filepath})")
                time.sleep(0.1)
            except OSError as e:
                raise e
        return self
...
# 1. Í≥†Ï†ï ?§Ï†ï (Î°úÏª¨ ?åÏä§?∏Ïö© ?¥Îçî Í≤ΩÎ°ú Î∞?Î∂Ä?úÏõê Î¶¨Ïä§??
import sys
...
def load_staff_list():
    ...
    try:
        ...
        print(f"ÎßàÏä§???åÏùº Î°úÎìú ?§Ìå®: {e}")
        
    try:
        safe_write_json(STAFF_MASTER_FILE, default_staff)
    except Exception as e:
        print(f"ÎßàÏä§???åÏùº ?ùÏÑ± ?§Ìå®: {e}")
    return default_staff
...
def load_task_list():
    ...
    try:
        ...
        print(f"?∏Î? ?ÖÎ¨¥ ÎßàÏä§???åÏùº Î°úÎìú ?§Ìå®: {e}")
        
    try:
        safe_write_json(TASK_MASTER_FILE, default_tasks)
    except Exception as e:
        print(f"?∏Î? ?ÖÎ¨¥ ÎßàÏä§???åÏùº ?ùÏÑ± ?§Ìå®: {e}")
    return default_tasks
...
class SettingsWindow(QDialog):
    def __init__(self, app_window):
        ...
        self.tabs.addTab(self.tab_general, "Í∏∞Î≥∏ ?§Ï†ï")
        
        # Tab 2: Staff
        self.tab_staff = QWidget()
        self.setup_staff_tab()
        self.tabs.addTab(self.tab_staff, "?¥Îãπ??Í¥ÄÎ¶?)
        
        # Tab 3: Tasks
        self.tab_tasks = QWidget()
        self.setup_task_tab()
        self.tabs.addTab(self.tab_tasks, "?∏Î??ÖÎ¨¥ Í¥ÄÎ¶?)
        
        btn_frame = QFrame()
        btn_layout = QVBoxLayout(btn_frame)
        self.save_btn = QPushButton("?Ä??Î∞??´Í∏∞")
    ...
    def setup_general_tab(self):
        ...
        layout.addWidget(QLabel("?∞Ïù¥???ÅÏû¨ ?¥Îçî ?ÑÏπò:"))
        ...
        btn_browse = QPushButton("?¥Îçî Î≥ÄÍ≤?..")
        btn_browse.clicked.connect(self.browse_path)
        path_layout.addWidget(btn_browse)
        layout.addLayout(path_layout)
        
        btn_html = QPushButton("?Ä?úÎ≥¥??HTML ?Ä??)
        btn_html.clicked.connect(self.app.export_to_html)
        layout.addWidget(btn_html)
        
        btn_excel = QPushButton("?ëÏ?Î°?Ï∑®Ìï©/?Ä??)
    ...
    def browse_path(self):
        ...
        d = QFileDialog.getExistingDirectory(self, "?ÖÎ¨¥?ºÏ? ?Ä???¥Îçî ?†ÌÉù", LOCAL_BASE_PATH)
        if d:
    ...
    def add_category(self):
        ...
        if t:
            cat = QTreeWidgetItem([t, "Ïπ¥ÌÖåÍ≥†Î¶¨"])
    ...
    def add_task(self):
        ...
        if not items:
            QMessageBox.warning(self, "Í≤ΩÍ≥†", "Î®ºÏ? Ï∂îÍ???Ïπ¥ÌÖåÍ≥†Î¶¨Î•??†ÌÉù?¥Ï£º?∏Ïöî.")
            return
        item = items[0]
        if item.text(1) == "?∏Î??ÖÎ¨¥":
            item = item.parent()
    ...
    def save_settings(self):
        ...
        if new_path:
            ...
            try:
                ...
                QMessageBox.critical(self, "?§Î•ò", f"Í≤ΩÎ°ú ?§Ï†ï ?Ä???§Ìå®: {e}")
                return
        ...
        QMessageBox.information(self, "?Ä???ÑÎ£å", "?§Ï†ï???Ä?•Îêò?àÏäµ?àÎã§.\n?ïÏù∏???ÑÎ•¥Î©??ÑÎ°úÍ∑∏Îû®???¨Ïãú?ëÎê©?àÎã§.")
        self.accept()
...
class WorkLogApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("WORK log - ?¨Ïõê?êÍ? ?¥ÏõêÎ∂ÑÏÑù ?ÖÎ¨¥?ºÏ? (v1.0)")
    ...
    def startup_check(self):
        ...
        if not LOCAL_BASE_PATH or not os.path.exists(LOCAL_BASE_PATH):
            if LOCAL_BASE_PATH:
                QMessageBox.critical(self, "?ëÏÜç ?§Î•ò", f"?§Ï†ï???¥Îçî?Ä ?∞Í≤∞?òÏ? ?äÏïò?µÎãà??\nÍ≤ΩÎ°ú: {LOCAL_BASE_PATH}")
    ...
    def create_widgets(self):
        ...
        title_lbl = QLabel("?¨Ïõê?êÍ? ?¥ÏõêÎ∂ÑÏÑù ?ÖÎ¨¥?ºÏ?")
        font = QFont("Arial", 16, QFont.Bold)
        title_lbl.setFont(font)
        header_layout.addWidget(title_lbl)
        
        btn_settings = QPushButton("?òÍ≤Ω?§Ï†ï")
        btn_settings.clicked.connect(self.open_settings)
        header_layout.addWidget(btn_settings, 0, Qt.AlignRight)
        main_layout.addLayout(header_layout)
        
        # Info Box
        info_group = QGroupBox("?ºÏûê / ?¨Ïö©???†ÌÉù")
        info_layout = QHBoxLayout(info_group)
        
        info_layout.addWidget(QLabel("?ëÏóÖ ?†Ïßú:"))
        ...
        info_layout.addWidget(QLabel("?¥Îãπ???†ÌÉù:"))
        ...
        # Tasks & Other Box
        tasks_group = QGroupBox("?∏Î? ?ÖÎ¨¥ ?§Ï†Å (?ºÍ≥Ñ Í±¥Ïàò ?ÖÎ†•)")
        ...
        for cat, fields in TASK_CATEGORIES.items():
            ...
            # Ïπ¥ÌÖåÍ≥†Î¶¨ ?Ä?¥Ì??Ä ÏßÑÌïòÍ≤? ?¥Ïö©?Ä Í∏∞Î≥∏ ?∞Ìä∏Î°?
            ...
            normal_font = QFont("ÎßëÏ? Í≥†Îîï", 10)
        ...
        other_layout.addWidget(QLabel("[Í∑∏Ïô∏ ?ÖÎ¨¥ ÏßÑÌñâ?¨Ìï≠]"))
        ...
        btn_refresh = QPushButton("ÏµúÏã† ?∞Ïù¥??Î∂àÎü¨?§Í∏∞")
        btn_refresh.setMinimumHeight(40)
        btn_refresh.clicked.connect(self.refresh_data)
        btn_layout.addWidget(btn_refresh)
        
        btn_save = QPushButton("???§Ï†Å ?Ä?•ÌïòÍ∏?)
        btn_save.setMinimumHeight(40)
        btn_save.clicked.connect(self.save_data)
        btn_layout.addWidget(btn_save)
        
        btn_preview = QPushButton("ÎØ∏Î¶¨Î≥¥Í∏∞")
    ...
    def refresh_data(self):
        ...
        QMessageBox.information(self, "Î∂àÎü¨?§Í∏∞ ?ÑÎ£å", "?úÎ≤Ñ(?¥Îçî)???Ä?•Îêú ÏµúÏã† ?∞Ïù¥?∞Î? ?±Í≥µ?ÅÏúºÎ°?Î∂àÎü¨?îÏäµ?àÎã§.")

    def save_data(self):
        user_name = self.staff_combo.currentText()
        if not user_name or user_name == "?†ÌÉù?òÏÑ∏??:
            QMessageBox.warning(self, "Í≤ΩÍ≥†", "?¥Îãπ?êÎ? ?†ÌÉù??Ï£ºÏÑ∏??")
            return
        ...
        try:
            ...
            QMessageBox.information(self, "?Ä???±Í≥µ", f"{user_name}?òÏùò {date_input} ?§Ï†Å???Ä?•Îêò?àÏäµ?àÎã§.")
        except Exception as e:
            QMessageBox.critical(self, "?Ä???§Ìå®", f"?§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§: {e}")
    ...
    def _generate_html(self, is_export=False):
        ...
        for file_path in json_files:
            try:
                ...
                if data:
                    row = {"?ºÏûê": data.get("date", ""), "?¥Îãπ??: data.get("name", ""), "other_progress": data.get("other_progress", "")}
        ...
        user_name = self.staff_combo.currentText()
        if user_name and user_name != "?†ÌÉù?òÏÑ∏??:
            task_data = {}
            for unique_key, le in self.entries.items():
                val = le.text().strip()
                task_data[unique_key] = int(val) if val.isdigit() else 0
            
            ui_row = {"?ºÏûê": date_input, "?¥Îãπ??: user_name, "other_progress": self.other_progress_text.toPlainText().strip()}
            ui_row.update(task_data)
            
            replaced = False
            for i, r in enumerate(all_rows):
                if r.get("?ºÏûê") == date_input and r.get("?¥Îãπ??) == user_name:
                    all_rows[i] = ui_row
                    replaced = True
                    break
            if not replaced:
        ...
        if not all_rows:
            QMessageBox.information(self, "?àÎÇ¥", "?†Ìö®???∞Ïù¥?∞Í? ?ÜÏäµ?àÎã§.")
            return

        df_all = pd.DataFrame(all_rows)
        all_unique_users = df_all["?¥Îãπ??].unique().tolist()
        ordered_users = STAFF_LIST + [u for u in all_unique_users if u not in STAFF_LIST]

        df_daily = df_all[df_all["?ºÏûê"] == date_input]
        daily_dict = df_daily.set_index("?¥Îãπ??).to_dict('index') if not df_daily.empty else {}

        df_monthly_base = df_all[df_all["?ºÏûê"].str.startswith(folder_month)].copy()
        cols = [c for c in df_monthly_base.columns if c not in ['?ºÏûê', '?¥Îãπ??, 'other_progress']]
        df_monthly_base[cols] = df_monthly_base[cols].apply(pd.to_numeric, errors='coerce')
        monthly_dict = df_monthly_base.groupby("?¥Îãπ??).sum(numeric_only=True).to_dict('index')
        ...
        if out_file:
            ...
            if is_export:
                QMessageBox.information(self, "?ÑÎ£å", f"HTML Î¶¨Ìè¨?∏Í? ?ùÏÑ±?òÏóà?µÎãà??\n{out_file}")
        else:
            QMessageBox.critical(self, "?§Î•ò", "Î¶¨Ìè¨???åÎçîÎß??§Ìå®.")
    ...
    def export_to_excel(self):
        ...
        if not json_files:
            QMessageBox.information(self, "?àÎÇ¥", f"{folder_month} ?îÏóê ?Ä?•Îêú ?∞Ïù¥?∞Í? ?ÜÏäµ?àÎã§.")
            return
        ...
        df_daily = df_all[df_all["?ºÏûê"] == date_input].copy()
        if not df_daily.empty:
            df_daily.set_index("?¥Îãπ??, inplace=True)
            df_daily = df_daily.reindex(ordered_users)
            df_daily.index.name = "?¥Îãπ??
            df_daily.reset_index(inplace=True)
            df_daily["?ºÏûê"] = df_daily["?ºÏûê"].fillna(date_input)
            for col in df_daily.columns:
                if col in ["?ºÏûê", "?¥Îãπ??, "other_progress"]:
                    df_daily[col] = df_daily[col].fillna("")
                else:
                    df_daily[col] = df_daily[col].fillna(0)
        else:
            df_daily = pd.DataFrame(columns=["?ºÏûê", "?¥Îãπ??, "other_progress"] + [f"{c}_{f}" for c, fs in TASK_CATEGORIES.items() for f in fs])

        if "?ºÏûê" in df_all.columns:
            df_monthly_base = df_all.drop(columns=["?ºÏûê"])
        else:
        ...
        df_monthly = df_monthly_base.groupby("?¥Îãπ??).sum(numeric_only=True)
        df_monthly = df_monthly.reindex(ordered_users)
        df_monthly.index.name = "?¥Îãπ??
        ...
        try:
            with pd.ExcelWriter(excel_path, engine='xlsxwriter') as writer:
                ...
                df_monthly_with_total.to_excel(writer, sheet_name="?îÍ∞Ñ ?îÏïΩ", index=False)
                ...
                worksheet.write(0, 0, "?ÖÎ¨¥?ºÏ?", fmt_title)
                ...
                worksheet.merge_range(4, 0, 4, max_col, "Ï£???????, fmt_header_gray)
                worksheet.merge_range(5, 0, 8, max_col, memo_content, fmt_memo)
                    
                start_row = 10
                worksheet.merge_range(start_row, 0, start_row, max_col, "Í∞úÎ≥Ñ ?ÖÎ¨¥ ?ÑÌô©", fmt_header_gray)
                
                hr1 = start_row + 1
                hr2 = start_row + 2
                worksheet.merge_range(hr1, 0, hr2, 1, "?¥Îãπ??              \n               ?ÖÎ¨¥Î™?, fmt_header_diag)
                
                col = 2
                for staff in ordered_users:
                    worksheet.merge_range(hr1, col, hr1, col+1, staff, fmt_header_gray)
                    worksheet.write(hr2, col, "?ºÍ≥Ñ", fmt_header_gray)
                    worksheet.write(hr2, col+1, "?ÑÍ≥Ñ", fmt_header_gray)
                ...
                worksheet.merge_range(row, 0, row+4, 1, "Í∑∏Ïô∏\n?ÖÎ¨¥ ÏßÑÌñâ?¨Ìï≠", fmt_category)
                c = 2
                for staff in ordered_users:
                ...
                worksheet_monthly = writer.sheets['?îÍ∞Ñ ?îÏïΩ']
                ...
                chart_monthly.add_series({
                    'name': '??™©Î≥??îÍ∞Ñ Î∂Ä??Ï¥??ÖÎ¨¥??,
                    'categories': ['?îÍ∞Ñ ?îÏïΩ', 0, task_start_col, 0, task_end_col],
                    'values':     ['?îÍ∞Ñ ?îÏïΩ', last_row, task_start_col, last_row, task_end_col],
                    'data_labels': {'value': True}
                })
                chart_monthly.set_title({'name': f'?îÍ∞Ñ ??™©Î≥?Î∂Ä???§Ï†Å ({folder_month})'})
                chart_monthly.set_x_axis({'name': '?∏Î? ?ÖÎ¨¥'})
                chart_monthly.set_y_axis({'name': 'Ï¥??ÖÎ¨¥ Í±¥Ïàò'})
                
                worksheet_monthly.insert_chart(1, len(df_monthly_with_total.columns) + 1, chart_monthly)
                
            QMessageBox.information(self, "Ï∑®Ìï© ?±Í≥µ", f"Ï¥?{len(json_files)}Í±¥Ïùò ?åÏùº??Ï≤òÎ¶¨?òÏóà?µÎãà??\n\n?Ä??Í≤ΩÎ°ú:\n{excel_path}")
            ...
            try:
                with pd.ExcelWriter(excel_path) as writer:
                    ...
                    df_monthly_with_total.to_excel(writer, sheet_name="?îÍ∞Ñ ?îÏïΩ", index=False)
                QMessageBox.information(self, "Ï∑®Ìï© ?±Í≥µ (Í∏∞Î≥∏)", f"Ï¥?{len(json_files)}Í±¥Ïùò ?åÏùº??Ï≤òÎ¶¨?òÏóà?µÎãà??\n\n?Ä??Í≤ΩÎ°ú:\n{excel_path}")
                import subprocess
                if sys.platform == "win32":
                    os.startfile(excel_path)
                elif sys.platform == "darwin":
                ...
                QMessageBox.critical(self, "?§Î•ò", f"?ëÏ? ?Ä??Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§: {e}\n{e2}")
    ...
    def load_memo(self, silent=False):
        ...
        if os.path.exists(memo_path):
            try:
                ...
                if not silent: QMessageBox.information(self, "Î∂àÎü¨?§Í∏∞", "?±Í≥µ?ÅÏúºÎ°?Î∂àÎü¨?îÏäµ?àÎã§.")
            except: pass
    ...
    def save_memo(self):
        ...
        try:
            ...
            QMessageBox.information(self, "?Ä???±Í≥µ", "?±Í≥µ?ÅÏúºÎ°??Ä?•Îêò?àÏäµ?àÎã§.")
        except Exception as e:
            QMessageBox.critical(self, "?§Î•ò", str(e))


if __name__ == "__main__":
    app = QApplication(sys.argv)
    
    font = QFont("ÎßëÏ? Í≥†Îîï", 10)
    app.setFont(font)
